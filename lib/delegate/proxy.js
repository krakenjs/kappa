...
var proto = {
    ...
    proxy: function (req, reply) {
        ...
        child.on('response', this._onresponse.bind(this, req, reply));
        child.setTimeout(5000, child.abort.bind(child));
        req.raw.req.pipe(child);
    },

    _onresponse: function (outgoing, reply, incoming) {
        var cont, dest, self;

        self = this;

        function respond(body) {
            var response = reply(body).code(incoming.statusCode);
        ...
        dest.once('error', self._onerror.bind(this));
        dest.once('finish', self.oncomplete);
        ...
        if (incoming.headers['content-type'] && incoming.headers['content-type'].indexOf('application/json') > -1) {
            Nipple.read(incoming, function (err, body) {
                ...
                if (body instanceof Error) {
                    return reply(Hapi.error.internal('Server error', body));
                }
